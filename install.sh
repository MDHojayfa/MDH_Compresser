...
achievement_sound() {
    [[ "$ENABLE_SOUND" == true ]] && echo -ne "\007" && sleep 0.1 && echo -ne "\007"
}
...
cyclone_animation_ultimate() {
    ...
    local spiral_chars=('◐' '◓' '◑' '◒' '●' '○' '◉' '◎' '⊙' '⊚' '❂' '✺' '✹' '★')
    local angle=0
    local chaos_particles=()
    
    while (( SECONDS < end_time )); do
        # Don't clear - creates trail effect
        
        # Draw multiple spiral arms
        for (( arm=0; arm<6; arm++ )); do
            local arm_angle=$((angle + arm * 60))
            
            for (( r=2; r<20; r++ )); do
                # Calculate position using bc for floating point
                local rad_angle=$(echo "scale=4; ($arm_angle + $r * 25) * 3.14159 / 180" | bc 2>/dev/null || echo "0")
                local x=$(echo "scale=0; $center_x + $r * 2.5 * c($rad_angle)" | bc -l 2>/dev/null || echo "$center_x")
                ...
                if (( x > 0 && x < TERM_COLS && y > 0 && y < TERM_ROWS )); then
                    move_cursor $y $x
                    local char_idx=$(( (r + arm + angle/10) % ${#spiral_chars[@]} ))
        ...
        for (( d=0; d<5; d++ )); do
            local dx=$((RANDOM % TERM_COLS))
            local dy=$((RANDOM % TERM_ROWS))
            local debris=('*' '.' '·' '°' '×' '✧' '✦')
            move_cursor $dy $dx
...
loading_sequence() {
    ...
    loading_bar_electric 0.5 "Finalizing chaos"
...
compress_asian() {
    ...
    if [[ -d "$input" ]]; then
        local temp_tar="/tmp/mdh_asian_$$.tar"
...
compress_drunk() {
    ...
    if [[ -d "$input" ]]; then
        local temp_tar="/tmp/mdh_drunk_$$.tar"
        tar -C "$(dirname "$input")" -cf "$temp_tar" "$(basename "$input")"
...
compress_beast() {
    ...
    if [[ -d "$input" ]]; then
        local temp_tar="/tmp/mdh_beast_$$.tar"
        tar -C "$(dirname "$input")" -cf "$temp_tar" "$(basename "$input")"
...
compress_legendary() {
    ...
    if command -v lrzip &>/dev/null; then
        ...
        if [[ -d "$input" ]]; then
            local temp_tar="/tmp/mdh_legendary_$$.tar"
            tar -C "$(dirname "$input")" -cf "$temp_tar" "$(basename "$input")"
...
handle_transfer() {
    ...
    echo ""
    echo -e "  ${DIM}═══ LOCAL/DIRECT ═══${RESET}"
    echo -e "  ${NEON_YELLOW}[5]${RESET} 🏠 Local Network (Python server)"
    echo ""
...
main() {
    ...
    # Boot sequence (only on first run)
