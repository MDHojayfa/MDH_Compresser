...
compress_flash() {
    ...
    echo ""
    if [ -d "$input" ]; then
        tar -cf - "$input" 2>/dev/null | pv -pterb -s "$original_size" 2>/dev/null | lz4 -1 -f - "${output}" 2>/dev/null || \
...
compress_flower() {
    ...
    echo ""
    if [ -d "$input" ]; then
        tar -cf - "$input" 2>/dev/null | pv -pterb -s "$original_size" 2>/dev/null | zstd -3 -f -o "${output}" 2>/dev/null || \
        tar -cf - "$input" | zstd -3 -f > "${output}"
    else
...
compress_godly() {
    ...
    if [ -d "$input" ]; then
        tar -cf - "$input" 2>/dev/null | pv -pterb -s "$original_size" 2>/dev/null | xz -9 -e -f > "${output}" 2>/dev/null || \
...
compress_legendary() {
    ...
    if command -v lrzip &> /dev/null; then
        ...
        if [ -d "$input" ]; then
            tar -cf - "$input" 2>/dev/null | pv -pterb -s "$original_size" 2>/dev/null | lrzip -z -L 9 > "${output}" 2>/dev/null || \
        ...
        if [ -d "$input" ]; then
            tar -cf - "$input" 2>/dev/null | pv -pterb -s "$original_size" 2>/dev/null | xz -9 -e --memlimit=50% > "${output}" 2>/dev/null || \
...
compress_mythic() {
    ...
    echo ""
    if [ -d "$input" ]; then
        tar -cf - "$input" 2>/dev/null | pv -pterb -s "$original_size" -N "Pass 1" 2>/dev/null | zstd -19 -f > "${temp_file}.zst" 2>/dev/null || \
        tar -cf - "$input" 2>/dev/null | zstd -19 -f > "${temp_file}.zst"
    else
...
boot_sequence() {
    ...
    bios_message "RAM detected: ${ram_total}MB" "OK"
...
show_main_menu() {
    ...
    echo -e "  ${DIM}🕐 ${current_time} | 📅 ${current_date} | 🎖️ Level: ${level_name} | ⭐ XP: ${xp}${RESET}"
    echo ""
    ...
    echo -e "  ${DIM}💾 RAM: ${ram_used}/${ram_total}MB | 💿 Free: ${disk_free}MB${RESET}"
    echo ""
    echo -e "${NEON_GREEN}  ╔════════════════════════════════════════════════════════════╗${RESET}"
