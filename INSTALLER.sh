...
compress_flash() {
    ...
    echo ""
    if [ -d "$input" ]; then
        # Use -C to change to parent directory, then compress by name only
...
boot_sequence() {
    ...
    # Show loading bar
    echo -e "  ${DIM}Initializing MDH_COMPRESSER...${RESET}"
    echo ""
    for i in {1..50}; do
        local pct=$((i * 2))
        printf "\r  ${NEON_GREEN}["
    ...
    bios_message "Disk free: ${disk_free}MB" "OK"
    bios_message "Time synced: ${current_time}" "OK"
